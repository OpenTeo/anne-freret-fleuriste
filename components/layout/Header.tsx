  ...
  return (
    <>
      <header className={`fixed left-0 right-0 z-50 transition-all duration-500 ${
        isScrolled ? 'top-0 bg-[#faf8f5]/95 backdrop-blur-md' : 'top-7 bg-[#faf8f5] md:bg-transparent'
      }`}>
        {/* Trust bar - sous le header */}
        <div className={`border-b border-[#e8e0d8] bg-[#faf8f5] transition-all duration-500 ${isScrolled ? 'h-0 overflow-hidden opacity-0' : 'h-auto opacity-100'}`}>
          <div className="flex items-center justify-center gap-2 py-1">
            <span className="text-[10px] md:text-[11px] text-[#2d2a26]/50">4.8 / 5</span>
            <div className="flex">
              {[1,2,3,4,5].map(s => (
                <svg key={s} className={`w-3 h-3 ${s <= 4 ? 'text-[#c4a47a]' : 'text-[#e8e0d8]'}`} fill="currentColor" viewBox="0 0 20 20">
                  <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                </svg>
              ))}
            </div>
            <span className="text-[10px] md:text-[11px] text-[#2d2a26]/50 uppercase tracking-wider">238 avis clients</span>
          </div>
        </div>
