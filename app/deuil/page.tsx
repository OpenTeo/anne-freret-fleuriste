              ...
              {filtered.map(product => (
                <Link key={product.slug} href={`/produit/${product.slug}`} className="group">
                  ...
                  <div className="flex gap-1.5 mb-1.5">
                    {product.sizes?.map(s => (
                      <span key={s.name} className="text-[8px] md:text-[9px] text-[#2d2a26]/30 uppercase tracking-wider">
                        {s.name}
                      </span>
                    ))}
                  </div>
                  <p className="text-[#c4a47a] text-sm">À partir de {product.sizes?.[0]?.price.toFixed(2)}€</p>
                </Link>
              ))}
            </div>
